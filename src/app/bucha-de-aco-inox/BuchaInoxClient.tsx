...
import {
  Settings, ShieldCheck, CheckCircle, Users, FileText, PenTool, Factory, Globe, HardHat,
  Thermometer, Phone, Mail, MapPin, Send, Plus, Layers, Award, Search, Loader2,
} from 'lucide-react';
...
import { buchaCopy } from './bucha-copy';

const stripIcons = [Settings, PenTool, Globe, ShieldCheck, Users];
const differentialIcons = [Award, Settings, ShieldCheck, CheckCircle, Users, FileText, Factory, Globe, HardHat];
const applicationIcons = [HardHat, Factory, Layers, Thermometer, Thermometer, Factory, Settings];
const galleryImages = [
  '/images/aceros/buchas/bucha-aco-inox-aceros-01.jpg',
  '/images/aceros/buchas/bucha-aco-inox-aceros-02.jpg',
  '/images/aceros/buchas/bucha-aco-inox-aceros-03.jpg',
  '/images/aceros/buchas/bucha-aco-inox-aceros-04.jpg',
  '/images/aceros/buchas/bucha-aco-inox-aceros-05.jpg',
  '/images/aceros/buchas/bucha-aco-inox-aceros-06.jpg',
];

export function BuchaInoxClient() {
  const { toast } = useToast();
  const { language } = useLanguage();
  const c = buchaCopy[language] ?? buchaCopy.pt;
  ...
  return (
    <div className="bg-background text-foreground font-body overflow-x-hidden">
      ...
      <section className="relative isolate min-h-[calc(100svh-80px)] overflow-hidden bg-[#07121e] text-white">
        <Image src="/images/aceros/buchas/bucha-aco-inox-aceros-02.jpg" alt={c.galleryAlts[1]} fill sizes="100vw" className="object-cover object-center opacity-70" priority />
          ...
          <div className="w-full max-w-[920px]">
            ...
            <div className="mt-7 flex animate-fade-in-up flex-col gap-3 sm:mt-10 sm:flex-row">
              <Button asChild size="lg" className="h-14 rounded-none bg-accent px-8 text-xs font-bold uppercase tracking-[0.12em] text-white hover:bg-[#cf5f11] sm:h-16">
                <Link href="#form-landing-bucha">{c.ctaRequestQuote}</Link>
              </Button>
              <Link href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" className="track-whatsapp inline-flex h-14 items-center justify-center gap-2 whitespace-nowrap rounded-none border border-white/35 bg-white/5 px-8 text-xs font-bold uppercase tracking-[0.12em] text-white backdrop-blur-sm transition-colors hover:border-white hover:bg-white hover:text-[#07121e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white sm:h-16">
                {c.ctaWhatsappTeam}
              </Link>
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-lg text-muted-foreground leading-relaxed mb-6">
              {c.introP1a}<strong>{c.introP1b}</strong>
            </p>
            <p className="text-lg text-muted-foreground leading-relaxed">
              {c.introP2a}<strong>{c.introP2b}</strong>
            </p>
          </div>
          <div className="relative aspect-square rounded-2xl overflow-hidden shadow-2xl">
            <Image src="/images/aceros/buchas/bucha-aco-inox-aceros-03.jpg" alt={c.galleryAlts[2]} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-white py-14 sm:py-20">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            ...
            <p className="text-muted-foreground mt-4 max-w-2xl mx-auto">{c.differentialsLead}</p>
          </div>
          <div className="grid grid-cols-1 items-stretch gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {c.differentials.map((item, i) => {
              const Icone = differentialIcons[i] ?? Award;
              return (
      ...
      <section className="bg-primary py-14 text-white sm:py-20">
        <div className="container mx-auto px-4">
          ...
          <p className="mt-16 text-center text-slate-300 max-w-3xl mx-auto">{c.applicationsFooter}</p>
        </div>
      </section>
                ...
                <Dialog key={url}>
                  <DialogTrigger asChild>
                    <button type="button" aria-label={`${c.zoomAria}: ${alt}`} className="group relative block aspect-square w-full cursor-pointer overflow-hidden rounded-xl bg-gray-100 text-left shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2">
                      <Image src={url} alt="" fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-110" />
                  ...
                  <DialogContent className="max-w-4xl p-4">
                    ...
                    <div className="relative mt-4 aspect-video w-full"><Image src={url} alt={alt} fill sizes="(max-width: 896px) 95vw, 896px" className="object-contain" /></div>
                  </DialogContent>
                </Dialog>
              );
            })}
        ...
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="order-2 md:order-1 relative aspect-video rounded-3xl overflow-hidden shadow-2xl">
              <Image src="/images/aceros/buchas/bucha-aco-inox-aceros-05.jpg" alt={c.galleryAlts[4]} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" />
            </div>
            <div className="order-1 md:order-2">
              <h2 className="font-headline text-3xl md:text-4xl font-bold text-slate-900 mb-8 uppercase">{c.whyTitle}</h2>
      ...
      <section className="bg-[#b54b00] py-14 text-white sm:py-20">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Button asChild size="lg" className="bg-primary text-white hover:bg-black px-12 py-8 text-xl"><Link href="#form-landing-bucha">{c.ctaMidRequest}</Link></Button>
            <Button asChild size="lg" variant="outline" className="border-primary text-primary hover:bg-primary hover:text-white px-12 py-8 text-xl">
              <Link href={`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`} target="_blank" rel="noopener noreferrer" className="track-whatsapp">{c.ctaMidWhatsapp}</Link>
            </Button>
          </div>
        </div>
      </section>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div><label htmlFor="bucha-phone" className="text-sm font-bold text-slate-700 block mb-2">{c.formPhone} *</label><Input id="bucha-phone" name="phone" type="tel" inputMode="tel" autoComplete="tel" placeholder="(00) 00000-0000" required className="bg-slate-50 border-slate-200" disabled={isSubmitting} /></div>
                    <div><label htmlFor="bucha-city" className="text-sm font-bold text-slate-700 block mb-2">{c.formCity}</label><Input id="bucha-city" name="city" autoComplete="address-level1" placeholder={c.formCityPh} className="bg-slate-50 border-slate-200" disabled={isSubmitting} /></div>
                  </div>
                  <div><label htmlFor="bucha-email" className="text-sm font-bold text-slate-700 block mb-2">{c.formEmail} *</label><Input id="bucha-email" name="email" type="email" autoComplete="email" placeholder={c.formEmailPh} required className="bg-slate-50 border-slate-200 track-email" disabled={isSubmitting} /></div>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div><label htmlFor="bucha-material" className="mb-2 block text-sm font-bold text-slate-700">{c.formMaterial}</label><Input id="bucha-material" name="material" placeholder={c.formMaterialPh} className="border-slate-200 bg-slate-50" disabled={isSubmitting} /></div>
                    <div><label htmlFor="bucha-deadline" className="mb-2 block text-sm font-bold text-slate-700">{c.formDeadline}</label><Input id="bucha-deadline" name="deadline" placeholder={c.formDeadlinePh} className="border-slate-200 bg-slate-50" disabled={isSubmitting} /></div>
                  </div>
                  <div><label htmlFor="bucha-dimensions" className="mb-2 block text-sm font-bold text-slate-700">{c.formDimensions} *</label><Input id="bucha-dimensions" name="dimensions" placeholder={c.formDimensionsPh} required className="border-slate-200 bg-slate-50" disabled={isSubmitting} /></div>
                  <div><label htmlFor="bucha-drawing" className="mb-2 block text-sm font-bold text-slate-700">{c.formDrawing}</label><Input id="bucha-drawing" name="drawing" type="file" accept=".pdf,.dwg,.dxf,.step,.stp,.iges,.igs,.jpg,.jpeg,.png" className="h-auto border-slate-200 bg-slate-50 py-2 text-sm" disabled={isSubmitting} /><p className="mt-2 text-xs leading-5 text-slate-500">{c.formDrawingHelp}</p></div>
                  <div><label htmlFor="bucha-message" className="text-sm font-bold text-slate-700 block mb-2">{c.formMessage}</label><Textarea id="bucha-message" name="message" placeholder={c.formMessagePh} rows={4} className="bg-slate-50 border-slate-200" disabled={isSubmitting} /></div>
                </div>
                <label className="flex items-start gap-3 text-xs leading-relaxed text-slate-600">
                  <input type="checkbox" id="bucha-privacy" name="privacy" required disabled={isSubmitting} className="mt-0.5 h-4 w-4 shrink-0 accent-orange-500" />
